...
function AppCard({ name, title, address, experience, imageUrl }) {
  return (
    <div className="border p-2 rounded-xl">
      {imageUrl && <img src={imageUrl} alt={title} className="rounded-lg h-[300px] w-full" />}
      <h2 className="text-md bg-blue-200 w-[150px] text-center rounded-2xl mt-2 p-1 text-blue-700 font-semibold">{title}</h2>
